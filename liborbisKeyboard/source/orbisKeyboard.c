...
static void orbisKeyboardEventUpdate(void *arg,const OrbisKeyboardEvent *e)
{
	...
	switch(e->id)
	{
		case ORBIS_KEYBOARD_EVENT_OPEN:
        	sys_log("liborbiskeyboard opened for userId:0x%08x\n",e->param.resourceIdArray.userId);
			for(i=0;i<3;i++)
			{
        		sys_log("%d ",e->param.resourceIdArray.resourceId[i]);
			}
    		sys_log("\n");			
			break;
		...
		case ORBIS_KEYBOARD_EVENT_KEYCODE_UP:
			sys_log("liborbiskeyboard key up for keycode:0x%04x\n",e->param.keycode.keycode);
			orbisKeybordGetSpecialKeys(e);
			if(e->param.keycode.keycode<0x001e || e->param.keycode.keycode==0x0033)
			{
				index=orbisKeyboardConf->caps_key|orbisKeyboardConf->shift_key_left|orbisKeyboardConf->shift_key_right;
		...
		case ORBIS_KEYBOARD_EVENT_KEYCODE_REPEAT:
			sys_log("liborbiskeyboard key repeat for keycode:0x%04x\n",e->param.keycode.keycode);
			break;
		case ORBIS_KEYBOARD_EVENT_CONNECTION:
    		sys_log("liborbiskeyboard connected for userId:0x%08x\n",e->param.resourceIdArray.userId);
			for(i=0;i<3;i++)
			{
    			sys_log("%d ",e->param.resourceIdArray.resourceId[i]);
			}
    	...
    	sys_log("liborbiskeyboard disconnected for userId:0x%08x\n",e->param.resourceIdArray.userId);
		for(i=0;i<3;i++)
		{
    		sys_log("%d ",e->param.resourceIdArray.resourceId[i]);
		}
    	...
    	sys_log("liborbiskeyboard aborted\n");
		
			break;
		default:
    	sys_log("liborbiskeyboard unknown event\n");
...
void orbisKeyboardClose()
{
	...
	if(orbisKeyboardConf!=NULL)
	{
		...
		if(orbisKeyboardConf->status==0 )
		{
			...
			if(ret<0)
			{
		        sys_log("liborbiskeyboard sceImeKeyboardClose return error 0x%8x\n",ret);
...
{
    sys_log("liborbiskeyboard sceImeKeyboardOpen in\n");
	sleep(1);
	OrbisKeyboardParam param;
	int ret;
	if(orbisKeyboardConf!=NULL)
	{
	    sys_log("liborbiskeyboard orbisKeyboardConf not null \n");
		
		if(orbisKeyboardConf->orbiskeyboard_initialized!=1  )
		{
		    sys_log("liborbiskeyboard orbisKeyboardConf %d \n",orbisKeyboardConf->orbiskeyboard_initialized);
		...
		if(ret<0)
		{
	        sys_log("liborbiskeyboard sceImeKeyboardOpen return error 0x%8x\n",ret);
	        return -1;
		}
		sleep(1);
        sys_log("liborbiskeyboard sceImeKeyboardOpen return success %d\n",ret);
		orbisKeyboardConf->status=0; //opened
		return 0;
	}
	else
	{
        sys_log("liborbiskeyboard orbisKeyboardOpen error config null\n");
...
int orbisKeyboardInit()
{
	...
	if (orbisKeyboardConf->orbiskeyboard_initialized==1) 
	{
		sys_log("liborbiskeyboard is already initialized!\n");
		return orbisKeyboardConf->orbiskeyboard_initialized;
	}
	...
	if(ret<0)
	{
        sys_log("liborbiskeyboard sceSysmoduleLoadModule return error 0x%8x\n",ret);
        return -1;
	}
	...
	if(ret<0)
	{
        sys_log("liborbiskeyboard sceUserServiceGetInitialUser return error 0x%8x\n",ret);
        return -1;
	...
	sys_log("liborbiskeyboard is initialized!\n");
